...
export default {
  ...
  methods: {
	...
	this.ruleForm.neirongxiangqing = this.ruleForm.neirongxiangqing.replace(reg,'../../../ssm06oin/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhishiziliao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhishiziliaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
